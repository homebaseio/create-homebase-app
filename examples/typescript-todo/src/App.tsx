...
import { HomebaseProvider, useEntity, useTransact, useQuery, Entity } from 'homebase-react'
import './App.css'
  ...
  return (
    <div>
      ...
      <ProjectSelect
        ...
        onChange={(project:number) => transact([{ todoFilter: { id: filters.get('id'), project }}])}
      />
    </div>
  )
}

const ProjectSelect = ({ value, onChange }: {value:any, onChange:any}) => {
...
const TodoProject = ({ todo }: {todo: Entity})  => {
  ...
  return (
    <ProjectSelect
      ...
      onChange={(projectId: number) => transact([{ todo: { id: todo.get('id'), 'project': projectId || null }}])}
    />
  )
}
